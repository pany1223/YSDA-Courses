...
def get_unique_user_ids_visited_page_after_ts(
    ...
    """
    return {x['UserID'] for x in records if x['EventTime'] > ts and x['PageID'] == page_id}
...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    return [x['RegionID'] if x['RegionID'] is not None else DEFAULT_REGION_ID for x in records]
