...
class Reduce(Operation):
    ...
    def __call__(self, rows: TRowsIterable, *args: tp.Any, **kwargs: tp.Any) -> TRowsGenerator:
        for k, g in groupby(rows, None):
            yield from self.reducer(tuple(k), g)
...
class Split(Mapper):
    ...
    def __call__(self, row: TRow) -> TRowsGenerator:
        ...
        for key in row.keys():
            if key == self.column:
                splitted_values = row[self.column].split(self.separator)
                for splitted_value in splitted_values:
                    yield res_dict | {key: splitted_value}
            else:
                res_dict.update({key: row[key]})
...
class TopN(Reducer):
    ...
    def __call__(self, group_key: tuple[str, ...], rows: TRowsIterable) -> TRowsGenerator:
        yield from heapq.nlargest(self.n, g, key=lambda x: str(x[self.column_max]))
